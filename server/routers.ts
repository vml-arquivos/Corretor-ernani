...
import { interestRates } from "../drizzle/schema";
...
const integrationRouter = router({
  ...
  // Analisar conversa WhatsApp e atualizar perfil do lead automaticamente
  analyzeConversation: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const { analyzeConversation, mapAnalysisToLeadData } = await import("./conversationAnalyzer");

        // Buscar lead pelo telefone
        const allLeads = await db.getAllLeads();
        const lead = allLeads.find(l => l.phone === input.phone || l.whatsapp === input.phone);
        ...
        const messages = await db.getMessagesByPhone(input.phone);

        if (!messages || messages.length === 0) {
          return { success: false, error: "Nenhuma mensagem encontrada para este telefone" };
        }

        // Converter mensagens para o formato esperado pelo analisador
        const conversationMessages = messages
          .filter(msg => msg.content) // Filtrar mensagens sem conte\u00fado
          .map(msg => ({
            role: msg.type === "incoming" ? "user" as const : "assistant" as const,
            content: msg.content!,
            timestamp: msg.timestamp,
          }));

        // Analisar conversa
        const analysis = await analyzeConversation(conversationMessages);

        // Mapear an\u00e1lise para dados do lead
        const leadUpdateData = mapAnalysisToLeadData(analysis);

        // Mapear qualifica\u00e7\u00e3o do formato ingl\u00eas para portugu\u00eas
        ...
        const intentMap = {
          ...
          rent: "locacao",
          both: "ambos",
          unknown: "venda", // padr\u00e3o
        };

        // Atualizar lead com os dados extra\u00eddos
        await db.updateLead(lead.id, {
          qualification: qualificationMap[analysis.qualification] as any,
          urgencyLevel: urgencyMap[analysis.urgency] as any,
          transactionInterest: intentMap[analysis.intent] as any,
          budgetMin: leadUpdateData.budgetMin,
          budgetMax: leadUpdateData.budgetMax,
          preferredPropertyTypes: leadUpdateData.preferredPropertyTypes,
          preferredNeighborhoods: leadUpdateData.preferredNeighborhoods,
          notes: `${lead.notes || ""}\n\n[An\u00e1lise Autom\u00e1tica - ${new Date().toLocaleString("pt-BR")}]\n${leadUpdateData.notes}`,
        });
        ...
        await db.createWebhookLog({
          ...
          payload: JSON.stringify({ phone: input.phone, messagesCount: messages.length }),
          response: JSON.stringify({ success: true, leadId: lead.id, analysis }),
          status: "success",
        });
        ...
        await db.createWebhookLog({
          source: "system",
          event: "conversation_analysis_error",
...
const simulatorRouter = router({
  ...
  getRates: publicProcedure
    .query(async () => {
      ...
      return await dbInstance.select()
        .from(interestRates)
        .where(sql`startDate <= ${today} AND (endDate IS NULL OR endDate >= ${today})`);
    }),

  // Criar/Atualizar Taxa de Juros (protegido - apenas admin)
  upsertRate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') {
        throw new Error('Apenas administradores podem gerenciar taxas de juros');
      }
      ...
      if (id) {
        await dbInstance.update(interestRates).set(data).where(eq(interestRates.id, id));
        return { success: true, id };
      } else {
        const result = await dbInstance.insert(interestRates).values(data);
        // MySQL returns insertId on insert
        return { success: true, id: result[0].insertId };
      }
    }),
    
  // Deletar Taxa de Juros (protegido - apenas admin)
  deleteRate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const dbInstance = getDb();
      await dbInstance.delete(interestRates).where(eq(interestRates.id, input.id));
      return { success: true };
    }),
});
...
const rentalRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      return await db.createRental(input);
    }),

  // Listar aluguéis (protegido)
  list: protectedProcedure
    .input(z.object({
      propertyId: z.number().optional(),
    ...
    .query(async ({ input }) => {
      return await db.getAllRentals(input);
    }),

  // Obter aluguel por ID (protegido)
  getById: protectedProcedure
    .input(z.object({ id: z.number() }))
    .query(async ({ input }) => {
      return await db.getRentalById(input.id);
    }),

  // Atualizar aluguel (protegido)
  update: protectedProcedure
    .input(z.object({
      id: z.number(),
      data: z.object({
        ...
        commissionPercentage: z.number().optional(),
        status: z.enum(['ativo', 'finalizado', 'cancelado', 'suspenso']).optional(),
      }),
    }))
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      await db.updateRental(input.id, input.data);
      return { success: true };
    }),

  // Deletar aluguel (protegido)
  delete: protectedProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
  ...
  createPayment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      return await db.createRentalPayment({ ...input, status: 'pendente' });
    }),
  ...
  getPayments: protectedProcedure
    ...
    .query(async ({ input }) => {
      return await db.getRentalPaymentsByRentalId(input.rentalId);
    }),

  // Atualizar pagamento (protegido)
  updatePayment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      await db.updateRentalPayment(input.id, { status: input.status, paidDate: input.paidDate as any });
      return { success: true };
    }),

  // Criar despesa (protegido)
  createExpense: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      return await db.createRentalExpense({ ...input, status: 'pendente' });
    }),

  // Listar despesas (protegido)
  getExpenses: protectedProcedure
    .input(z.object({ rentalId: z.number() }))
    .query(async ({ input }) => {
      return await db.getRentalExpensesByRentalId(input.rentalId);
    }),

  // Criar contrato (protegido)
  createContract: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== 'admin') throw new Error('Apenas admin');
      return await db.createRentalContract({ ...input, status: 'rascunho' });
    }),

  // Obter contrato (protegido)
  getContract: protectedProcedure
    .input(z.object({ rentalId: z.number() }))
    .query(async ({ input }) => {
      return await db.getRentalContractByRentalId(input.rentalId);
    }),
});
