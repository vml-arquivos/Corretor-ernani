...
export async function createRental(rental: InsertRental) {
  ...
  const insertId = Number(result[0].insertId);
  const created = await db.select().from(rentals).where(eq(rentals.id, insertId)).limit(1);
  return created[0]!;
}
...
export async function getAllRentals(filters?: {
  ...
  if (filters) {
    if (filters.propertyId) conditions.push(eq(rentals.propertyId, filters.propertyId));
    if (filters.tenantId) conditions.push(eq(rentals.tenantId, filters.tenantId));
    if (filters.ownerId) conditions.push(eq(rentals.ownerId, filters.ownerId));
    if (filters.status) conditions.push(eq(rentals.status, filters.status as any));
  }
  ...
  const result = await query.orderBy(desc(rentals.createdAt));
  return result;
}

export async function updateRental(id: number, rental: Partial<InsertRental>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.update(rentals).set(rental).where(eq(rentals.id, id));
}
...
// ============================================
// RENTAL PAYMENT FUNCTIONS
// ============================================

export async function createRentalPayment(payment: InsertRentalPayment) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(rentalPayments).values(payment);
  const insertId = Number(result[0].insertId);
  const created = await db.select().from(rentalPayments).where(eq(rentalPayments.id, insertId)).limit(1);
  return created[0]!;
}

export async function getRentalPaymentsByRentalId(rentalId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updateRentalPayment(id: number, payment: Partial<InsertRentalPayment>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.update(rentalPayments).set(payment).where(eq(rentalPayments.id, id));
}

// ============================================
// RENTAL EXPENSE FUNCTIONS
// ============================================

export async function createRentalExpense(expense: InsertRentalExpense) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(rentalExpenses).values(expense);
  const insertId = Number(result[0].insertId);
  const created = await db.select().from(rentalExpenses).where(eq(rentalExpenses.id, insertId)).limit(1);
  return created[0]!;
}

export async function getRentalExpensesByRentalId(rentalId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const result = await db
    .select()
    .from(rentalExpenses)
    .where(eq(rentalExpenses.rentalId, rentalId))
    .orderBy(desc(rentalExpenses.expenseDate));

  return result;
}

export async function updateRentalExpense(id: number, expense: Partial<InsertRentalExpense>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.update(rentalExpenses).set(expense).where(eq(rentalExpenses.id, id));
}
...
export async function getRentalContractByRentalId(rentalId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const result = await db
    ...
    .where(eq(rentalContracts.rentalId, rentalId))
    .limit(1);

  return result.length > 0 ? result[0] : null;
}

export async function updateRentalContract(id: number, contract: Partial<InsertRentalContract>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db.update(rentalContracts).set(contract).where(eq(rentalContracts.id, id));
}
